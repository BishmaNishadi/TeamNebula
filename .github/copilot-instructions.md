...
- Read `testcases.csv`.
- Group cases by `Module` and create one Playwright spec file per module under `automation/tests/<module>.spec.ts` (kebab-case filename).
- Use the **Playwright MCP** browser tools to live-explore the app, discover locators, and produce **robust** tests using `getByRole`, `getByLabel`, `getByText` — avoid brittle CSS/XPath when possible.
- Run the tests via `npx playwright test`.
- For **every** test failure or behavior that doesn't match the test case's `Expected Result`, **append a row to `defects.csv`** with:
  - `Module`, `Title` (concise), `Severity` (Critical / High / Medium / Low based on impact), `Priority`, `Steps to Reproduce` (numbered), `Expected Result`, `Actual Result`, `Status` = Open, `Reported By` = "AI Agent".
- Also log defects for any UI/UX issues observed during exploration even if no test failed.

### Phase 4 — Strategy Document
- Read `testcases.csv`, `defects.csv`, and the files under `automation/tests/`.
- Write `strategy.md` with two top-level sections:
...
- `Type`: Functional / Negative / Boundary / UI / Integration
